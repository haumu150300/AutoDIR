...
def process_image(input_path, model, model_wrap, model_wrap_cfg, null_token, args, decoder_consistency=None):
    """Process a single image through the restoration pipeline."""
    
    # Reset artifact_category for each image
    global artifact_category
    artifact_category = [
        'noise', 'blur', 'rain', 'underexposure', 'haze', 'low resolution',
        'raindrop', 'no'
    ]
    ...
    for step_number in range(step_max_number):
        with torch.no_grad(), autocast("cuda"), model.ema_scope():
            ...
            if args.decoder_consistency and decoder_consistency:
                z = z / 0.18215
...
def main():
    ...
    parser.add_argument("--ckpt",
                        ...
                        type=str)
    parser.add_argument("--test-dir", default="demo", type=str,
                        help="Directory containing test images")
    parser.add_argument("--output", required=True, default="result", type=str)
    ...
    # Initialize decoder consistency if needed
    ...
    for ext in image_extensions:
        image_files.extend(glob(os.path.join(args.test_dir, ext)))
        image_files.extend(glob(os.path.join(args.test_dir, ext.upper())))
    ...
    for idx, image_path in enumerate(image_files, 1):
        ...
        try:
            process_image(image_path, model, model_wrap, model_wrap_cfg, null_token, args, decoder_consistency)
